...
export class AfiliadoComponent {
  ...
  buscarMedic() {  // busco medicos  y parseo la especialidad
    this.consultaBackApi.buscarMedic().subscribe((medicos: Medicos[]) => {
  ...
  procesarDisponibilidad(disponibilidad: Disponibilidad) {
    ...
    if (disponibilidad && disponibilidad.dias) {
      ...
      const hoursDoctor = this.generarHorasDisponibles(disponibilidad.horadesde, disponibilidad.horahasta);

      const medicoId = this.disponibilidad.dni_medico;
  ...
  generarDiasDisponibles(diasDisponibles: string[]): Date[] {
    ...
    for (let day = today; day <= end; day.setDate(day.getDate() + 1)) {
      ...
      console.log("diasDisponible ", diasDisponibles)
  ...
  getDayName(date: Date): string {
    const dayNames = ['LU', 'MA', 'MI', 'JU', 'VI', 'SA'];
  ...
  confirmarTurno(hour: any) {
    ...
    console.log("data sesion storage", sessionStorage.getItem('USER_DATA'));
    this.selectedHour = hour;
    if (this.selectedDate && this.selectedHour) {
      console.log("Hora seleccionadaaaaaaaaaa:", hour);
  ...
  confirmarYGuardarTurno() {
    ...
    const myData: string | null = sessionStorage.getItem('USER_DATA');

    if (myData) {
      try {

        const user = JSON.parse(myData);
        const pacienteId = user.dni;
        const dni_medico = this.disponibilidad.dni_medico;
        // Preparar el objeto del turno
        const turno = {
          dni_paciente: pacienteId, // ID del paciente
          dni_medico: dni_medico,
          // especialidad: this.especialidadSeleccionada,
          fechaturno: this.selectedDate,
    ...
    if (myData) {
      try {
        ...
        this.consultaBackApi.getHistorialTurnos(pacienteId, 1).subscribe((response) => {
          ...
          console.log("historial de turno", this.historialTurnos )
        }, (error) => {
  ...
  enviarCalificacion() {
    ...
    const id = this.turnoSeleccionado.id_turno;
    ...
    if (calificacion && id) {
      ...
      console.log("turnoSearch", turnoSearch)
