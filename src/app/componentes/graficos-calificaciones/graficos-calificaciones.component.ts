...
@Component({
  selector: 'app-graficos-calificaciones',
  standalone: true,
  imports: [],
  templateUrl: './graficos-calificaciones.component.html',
  styleUrl: './graficos-calificaciones.component.css'
})
export class GraficoCalificacionesComponent implements AfterViewInit {
  ...
  crearGrafico() {
    ...
    if (myData) {
      try {
        ...
        this.consultasService.getCalificacionesPorAfiliado(dni).subscribe(
          (response) => {
  ...
  calculateAverageCalifications(calificaciones: any[]): any[] {
    ...
    calificaciones.forEach(turno => {
        ...
        const id_Medico = turno.dni_medico;
